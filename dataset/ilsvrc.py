...
class ILSVRC2012(torch.utils.data.Dataset):
    def __init__(self, root, input_size, crop_size, train=True, transform=None, p_transform=None):
        self._root = root
        self._train = train
        self._input_size = input_size
        self._crop_size = crop_size
        self._transform = transform
        self._p_transform=p_transform
    ...
    def __getitem__(self, index):
        ...
        if not self._train:
            ...
            for bbox_i in range(len(bboxes)):
                ...
                bbox.insert(0, index)
                if not len(bbox)==5:
                    print('Anomely .......', len(bbox))
                newBboxes.append(bbox)
        image1=image
        # apply transformation
        if self._transform is not None:
            image = self._transform(image1)
        if self._p_transform is not None:
            image_p=self._p_transform(image1)
        if self._train:
            if self._p_transform is not None:
                return image, image_p, label, cls_name, img_name.split('/')[-1].split('.')[0]
            else:
                return image, label, cls_name, img_name.split('/')[-1].split('.')[0]
        else:
            #if not newBboxes.size()==(3, 448, 448):
                #print('Anomly found', newBboxes.size())

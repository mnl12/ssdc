...
def save_bbox_as_json(db_root, db_name, experiments, cnt, rank, bboxes, cls_name, image_name, phase='train'):
    offset = (480 - 448) / 2
    pred_bbox = {}
    for i in range(len(bboxes)):
        ...
        if db_name == 'cub':
            key = f'{db_root}/{phase}/{cls_name[i]}/{image_name[i]}.jpg'
            key = key.replace('//', '/')
            pred_bbox[key] = temp_save_box
        else:
            key = f'{db_root}/{phase}/{cls_name[i]}/{image_name[i]}.JPEG'
            key = key.replace('//', '/')
            pred_bbox[key] = temp_save_box
